    ...
    (function () {
      ...
      function summarizeMessageContent(content) {
        ...
        for (var i = 0; i < content.length; i++) {
          var part = content[i] || {};
          var type = part.type || '';
          if (type === 'input_text' && typeof part.input_text === 'string' && part.input_text.trim()) {
            texts.push(part.input_text.trim());
          }
          if ((type === 'input_file' || type === 'input_image' || type === 'input_video' || type === 'input_audio') && typeof part.filename === 'string' && part.filename.trim()) {
      ...
      function rHist() {
        ...
        for (var i = 0; i < state.hist.length; i++) {
          ...
          msg(x.role, summarizeMessageContent(content), i);
        }
      }
      function save() {
        try {
          var kept = state.hist.filter(function (x) {
            if (!x) return false;
      ...
      function load() {
        try {
          ...
          var a = JSON.parse(v);
          if (Array.isArray(a)) {
            state.hist = a.filter(function (x) {
              return x && (typeof x.content === 'string' || Array.isArray(x.content));
            });
          }
        } catch (_) {}
      }
      ...
      async function loadModels() {
        var prev = e.model.value;
        var headers = {};
        var key = (e.apiKey.value || '').trim();
        if (key) headers.Authorization = 'Bearer ' + key;
        try {
          st('正在加载模型列表...', 'ok');
          var resp = await fetch('/v1/models', { headers: headers });
          if (!resp.ok) {
            var t = await resp.text().catch(function(){ return ''; });
            throw new Error(emsg(resp.status, t));
          }
          ...
          if (prev && ids.indexOf(prev) >= 0) e.model.value = prev;
      ...
      function resendFromIndex(idx) {
        ...
        e.prompt.value = summarizeMessageContent(item.content);
        send(item.content, true, idx);
      }
      ...
      async function send(forceText, fromResend, replayFromIndex) {
        ...
        if (Array.isArray(forceText)) {
          ...
          if (!text && filesForSend.length === 0) { st('请输入内容或选择附件。', 'err'); return; }
        }
        ...
        if (!userMessageContent) {
          var content;
          try { st('正在处理附件...', 'ok'); content = await buildContent(text, filesForSend); }
          catch (err) { st(err && err.message || '附件处理失败', 'err'); return; }
        ...
        try {
          ...
          var resp = await fetch('/v1/chat/completions', {
            method: 'POST',
            headers: h,
            body: JSON.stringify({ model: model, stream: true, messages: payload }),
          ...
          while (!done) {
            ...
            buf += d.decode(ch.value || new Uint8Array(), { stream: !done });
            var lines = buf.split('\n');
            buf = lines.pop() || '';
            for (var i = 0; i < lines.length; i++) {
              ...
              try {
                ...
                if (typeof delta === 'string' && delta) {
                  out += delta;
                  aEl.textContent = 'Assistant: ' + out;
                  e.messages.scrollTop = e.messages.scrollHeight;
                }
              } catch (_) {}
            }
          }
          if (!out) { out = '(empty response)'; aEl.textContent = 'Assistant: ' + out; }
          state.hist.push({ role: 'user', content: userMessageContent });
          state.hist.push({ role: 'assistant', content: out });
          save();
          e.prompt.value = '';
          state.files = [];
          attRow();
          ...
          if (err && err.name === 'AbortError') st('已中断请求。', 'err');
          else {
            var m = err && err.message || '请求失败';
            st(m, 'err');
            if (!aEl.textContent) aEl.textContent = 'Assistant: [错误] ' + m;
          }
        } finally {
          state.ac = null;
          busy(false);
      ...
      e.files.onchange = function () {
        ...
        for (var i = 0; i < fs.length; i++) {
          if (state.files.length >= MAX) { st('最多允许 ' + MAX + ' 个附件。', 'err'); break; }
          var f = fs[i];
          if (f.size > MAX_SIZE) { st('文件过大：' + f.name + '（上限 ' + bfmt(MAX_SIZE) + '）', 'err'); continue; }
          state.files.push(f);
        }
        e.files.value = '';
        attRow();
        if (state.files.length) st('已选择 ' + state.files.length + ' 个附件。', 'ok');
      };
